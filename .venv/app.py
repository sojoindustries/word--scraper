...
# these are the words we wanna filter out cuz they show up too much
DOMAIN_STOP = {'line', 'shift', 'run', 'case', 'production', 'machine', 'ran', 'cans', 'cases'}
# mix our custom stopwords with the standard nltk ones
STOP_WORDS = set(stopwords.words('english')) | DOMAIN_STOP

# load the csv data and fix it up a bit
df = pd.read_csv('Data/CB_EOS_Data_Scraper.csv')
df['CUSTRECORD_EOS_MEMO'] = df['CUSTRECORD_EOS_MEMO'].fillna('')  # get rid of nans
df['DATE'] = pd.to_datetime(df['CUSTRECORD_EOS_DATE'])  # make the dates actual date objects

# grab all the unique product lines so we can filter by them
product_lines = sorted(df['BINNUMBER'].dropna().unique())


# this gets the most common words or phrases from the text
def extract_ngrams(texts, ngram_range=(1, 1), top_n=20):
    vectorizer = CountVectorizer(
        lowercase=True,
        token_pattern=r'\b[a-zA-Z]+\b',  # just look at letter words
    ...
    # sort by count and grab the top ones
    return pd.DataFrame({'phrase': features, 'count': counts}) \
        .sort_values('count', ascending=False) \
        .head(top_n)


# this finds important terms using that tf idf thing
...
# this does topic modeling to find themes in the text
def extract_lda_topics(texts, n_topics=5, n_top_words=5):
    vectorizer = CountVectorizer(stop_words=list(STOP_WORDS))
    X = vectorizer.fit_transform(texts)
    # train lda to find the main topics
...
def extract_clusters(texts, n_clusters=5):
    ...
    X = vectorizer.fit_transform(texts)
    # don't try to make more clusters than we have docs
    ...
    # count how many docs in each cluster
    label_counts = pd.DataFrame({'cluster': np.unique(labels), 'count': np.unique(labels, return_counts=True)[1]})
    return label_counts


# precalculate all the stats for each product line so dashboard runs faster
line_metrics = {}
...
# layout with all the dashboard controls and display areas
app.layout = html.Div([
    html.Div([
    ...
    html.Div([
        # loading spinners for each chart while they load
...
# this updates all the charts when you change the filter
@app.callback(
...
def update_dashboard(selected_line):
    ...
    # reusable func to make a table chart
    ...
    # chart showing downtime and jams over time
    sub_df = df[df['BINNUMBER'] == selected_line]
    # count days with downtime mentioned
    ...
    # count days with jams mentioned
